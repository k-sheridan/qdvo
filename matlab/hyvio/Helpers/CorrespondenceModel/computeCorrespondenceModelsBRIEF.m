...
for idx = (1:length(visibleLandmarkIDs))
    ...
    try
        [warpedPatch] = warpPatchToTargetFrame(graph.FrameContainer{frameIdx}.landmarks{landmarkIdx}, ...
            graph.FrameContainer{frameIdx}, frame, graph, orb.patchRadius);
    catch e
    ...
    try
        templateDescriptor = orb.evaluate(warpedPatch.image, [orb.patchRadius+1;orb.patchRadius+1]);
    catch e
        fprintf('Descriptor Extract Failed: %s\n', e.message);
        continue;
    end
...
%     regionMean = patch.meanIntensity;
    ...
    for dx = (-s.searchRadius:s.searchRadius)
        for dy = (-s.searchRadius:s.searchRadius)
            try
                targetDescriptor = orb.evaluate(frame.raw_image, centerPx + [dx;dy]);
            catch e
            ...
            % compare descriptors
            try
                [score] = orb.compare(templateDescriptor, targetDescriptor);
            catch e
                fprintf(1,'ham error! The message was:%s\n',e.message);
                continue;
            end
