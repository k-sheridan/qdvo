    ...
    methods
        ...
        function [residual, information, jacobians] = computeResidual(obj, graph)
            ...
            if nargout >= 3
                ...
                % Source Jacobian - imustate order: [dp, dphi, dv, dba, dbg]
                J_dt_o = projJac * dPi * (C'*A');
                J_dphi_o = -projJac * dPi * C'*A'*B * so3Hat(C*u0/dinv + f);
                jacobians.imustateJacobians{2} = {obj.landmarkObservation.landmarkParentFrameID, [J_dt_o, J_dphi_o, zeros(2, 9)]};
            end
