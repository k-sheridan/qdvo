...
class QuasiDirectErrorTerm_TargetFrame : public ArgMin::ErrorTermBase<ArgMin::Scalar<double>, ArgMin::Dimension<2>, ArgMin::VariableGroup<ArgMin::SE3>> {
public:
    ...
    QuasiDirectErrorTerm_TargetFrame( ArgMin::VariableKey<ArgMin::SE3> targetFrameVariableKey,   
                                Eigen::Matrix<double, 2, 2> information,
                                Graph* graph,
        ...
        this->correspondenceDistributionIndex = cdIndex;
    }
    ...
    void evaluate(ArgMin::VariableContainer<Variables...> &variables, bool relinearize = false)
    {
        ...
        if (relinearize)
        {
            Eigen::Matrix<double, 2, 6> &targetJacobian = (std::get<0>(variableJacobians));
