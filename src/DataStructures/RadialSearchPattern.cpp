...
QDVO::RadialSearchPattern::RadialSearchPattern(const unsigned maxRadius) {
  ...
  if (maxRadius % 2 == 0) {
    dim++;
  }  // ensure that the center of the array is an integer.
  dim += 2;
  ...
  mask.at(centerIdx).at(centerIdx) = true;
  ...
  for (int radius = 1; radius <= maxRadius; ++radius) {
    ...
    for (double theta = 0; theta < twoPi; theta += dTheta) {
      ...
      std::vector<Eigen::Vector2i> options{
          (Eigen::Vector2i(std::floor(dx), std::floor(dy))),
