...
class ErrorTermValidator
{
    ...
    bool validate(VariableContainer<Variables...>& variableContainer)
    {
        ...
        internal::static_for(errorTerm.variableKeys, [&](auto i, auto &key) {
            typedef typename std::tuple_element<i, typename internal::errorterm_validator_helper<typename ErrorTerm::VariableKeys>::variables>::type ThisVariable;
            using JacobianType = Eigen::Matrix<typename ErrorTerm::scalar_type, ErrorTerm::residual_dimension, ThisVariable::dimension>;
            ...
            {
                Eigen::Matrix<typename ErrorTerm::scalar_type, ErrorTerm::residual_dimension, 1> high, low;
                Eigen::Matrix<typename ThisVariable::scalar_type, ThisVariable::dimension, 1> dx = Eigen::Matrix<typename ThisVariable::scalar_type, ThisVariable::dimension, 1>::Zero();
