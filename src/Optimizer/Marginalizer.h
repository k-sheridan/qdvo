...
public:
    ...
    /// After marginalization, any error terms containing the variable will be erased.
    /// After marginalization, the prior should be cleared of all marginalized variables.
    /// Otherwise it will contain excess information.
    ///
    /// Ignored variables, if already uncorrelated in the prior, will remain uncorrelated after marginalization.
    /// This is done by ignoring their off diagonal terms when computing the schur complement of the
    /// unmarginalized variables block.
    ///
    /// @param ignoredVariableTypes Jacobians with respect to these variables types will be ignored.
    ...
    bool marginalizeVariable(VariableKey<VariableType> &marginalizedKey, GaussianPrior<Scalar<ScalarType>, VariableGroup<Variables...>> &prior, ErrorTermContainer<ErrorTerms...> &linearizedErrorTerms, VariableGroup<IgnoredVariables...> ignoredVariableTypes = VariableGroup<IgnoredVariables...>())
    {
        ...
        // Iterate through all error terms, and approximate the error terms which are a function of the marginalized variable with
        // a quadratic error term.
        //TODO I do not have to compute the notignored-ignored parts of the quadratic cost.
        internal::static_for(linearizedErrorTerms.tupleOfErrorTermMaps, [&](auto i, auto &errorTermMap) {
            ...
            if constexpr (internal::Is_in_tuple<VariableKey<MarginalizedVariable>, typename ThisErrorTerm::VariableKeys>::value)
            {
                // This error term type contains the same key type as the marginalized key.
                for (auto errorTermIt = errorTermMap.begin(); errorTermIt != errorTermMap.end(); errorTermIt++)
                {
                    ...
                    // If this error term is a function of the marginalized variable, add its approximation to the prior.
        ...
        // Compute the remaining part of the schur complement of the remaining variable block.
        // this is the A0_remain - B' * inv(A) * B

        // Get the marginalized row. This assumes that the row exists.
        auto &marginalizedRow = prior.A0.template getRowMap<MarginalizedVariable>().at(marginalizedKey);
        std::tuple<Variables *...> variableTuple;

        Eigen::Matrix<ScalarType, MarginalizedVariable::dimension, MarginalizedVariable::dimension> &A_marg = prior.A0.getBlock(marginalizedKey, marginalizedKey);

        Eigen::Matrix<ScalarType, MarginalizedVariable::dimension, MarginalizedVariable::dimension> A_margInv = A_marg.inverse();
        // Verify that the matrix inverse was valid.
        if (std::isnan(A_margInv.sum()))
        {
            SPDLOG_ERROR("Marginalization failed due to failed inversion of the marginalized block. Returning early.");
            return false;
        }

        internal::static_for(variableTuple, [&](auto i, auto &doNotUseMe) {
            typedef typename std::tuple_element<i, std::tuple<Variables...>>::type OuterVariable;

            // Don't use ignored variables.
            if constexpr (!internal::Is_in_tuple<OuterVariable, std::tuple<IgnoredVariables...>>::value)
            {
                auto &outerVariableMap = marginalizedRow.template getVariableMap<OuterVariable>();

                for (auto &outerPair : outerVariableMap)
                {
                    // Precompute B' * inv(A)
                    Eigen::Matrix<ScalarType, OuterVariable::dimension, MarginalizedVariable::dimension> bTAinv = (outerPair.second.transpose() * A_margInv).eval();

                    internal::static_for(variableTuple, [&](auto j, auto &doNotUseMeAlso) {
                        typedef typename std::tuple_element<j, std::tuple<Variables...>>::type InnerVariable;

                        // Don't use ignored variables.
                        if constexpr (!internal::Is_in_tuple<InnerVariable, std::tuple<IgnoredVariables...>>::value)
                        {
                            auto &innerVariableMap = marginalizedRow.template getVariableMap<InnerVariable>();

                            for (auto &innerPair : innerVariableMap)
                            {
                                // If the inner variable and outer variable are both the same type as the marginalized variable
                                // Check if they are the marginalized variable.
                                if constexpr (std::is_same<OuterVariable, MarginalizedVariable>::value && std::is_same<InnerVariable, MarginalizedVariable>::value)
                                {
                                    if (marginalizedKey == innerPair.first && marginalizedKey == outerPair.first)
                                    {
                                        // Skip this block.
                                        continue;
                                    }
                                    ...
                                    prior.A0.getBlock(outerPair.first, innerPair.first) -= bTAinv * innerPair.second;
                                }
                            }
                        }
                    });
