...
public:
    ...
    /**
     * Searches for the inverse depth of a landmark using a target keyframe. It is assumed that the target keyframe pose has already been estimated.
     * 
     * px(z) = Pi(T_target_source * (u * z))
     * d/dz px(z) = J_cameraModel * J_homogenousProjection * (R_target_source * u) = J_z
     * dz = resolution / norm(J_z) 
     *
     ...
     */
    void update(Graph& graph, Frame& sourceKeyframe, Frame& targetKeyframe, Landmark& landmark, PatchComparer& patchComparer, PatchWarper& patchWarper);
};
