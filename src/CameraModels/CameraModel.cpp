...
QDVO::CameraModel::CameraModel(SCALAR_TYPE fx, SCALAR_TYPE fy, SCALAR_TYPE cx,
                               SCALAR_TYPE cy, SCALAR_TYPE fov, int width,
                               int height)
    : cx(cx), cy(cy), fx(fx), fy(fy), fov(fov), width(width), height(height) {}

QDVO::Result<QDVO::Vector2> QDVO::CameraModel::project(
...
QDVO::Result<QDVO::Vector3> QDVO::CameraModel::unproject(
  ...
  return Eigen::Matrix<SCALAR_TYPE, 3, 1>((pixel(0) - this->cx) / this->fx,
                                          (pixel(1) - this->cy) / this->fy, 1);
}

bool QDVO::CameraModel::isPointPotentiallyVisible(
    const Eigen::Matrix<SCALAR_TYPE, 3, 1>& pointInCamera) const {
  if (pointInCamera(2) < SMALL_NUMBER) {
    return false;
  }
