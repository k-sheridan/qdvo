...
QDVO::EquidistantCameraModel::EquidistantCameraModel(
    SCALAR_TYPE fx, SCALAR_TYPE fy, SCALAR_TYPE cx, SCALAR_TYPE cy,
    SCALAR_TYPE fov, int width, int height,
    const Eigen::Vector4d &distortionCoeffs)
  ...
  this->radiusLookUpTable.umap.resize(
      std::ceil(this->fov / this->radiusLookUpTable.resolution));
...
QDVO::Result<QDVO::Vector2> QDVO::EquidistantCameraModel::project(
    Eigen::Matrix<SCALAR_TYPE, 3, 1> pointInCamera,
    Eigen::Matrix<SCALAR_TYPE, 2, 2> *projectionJacobian) const {
  if (!this->isPointPotentiallyVisible(pointInCamera)) {
    // throw std::runtime_error("point not possibly visible.");
    return {};
  }

  Eigen::Matrix<SCALAR_TYPE, 3, 1> homogenousPoint =
      pointInCamera / pointInCamera(2);

  SCALAR_TYPE normPointProjectedOntoImagePlane =
      sqrt(pointInCamera(0) * pointInCamera(0) +
           pointInCamera(1) * pointInCamera(1));
  SCALAR_TYPE theta =
      atan2(normPointProjectedOntoImagePlane, std::abs(pointInCamera(2)));
  ...
  Eigen::Matrix<SCALAR_TYPE, 2, 1> pixel(
      ...
      bearingDistorted(1) * this->fy + this->cy);
  ...
  if (projectionJacobian != nullptr) {
    try {
      ...
      projectionJacobian->block(0, 0, 2, 1) =
          this->project(homogenousPoint.block(0, 0, 3, 1) +
                        Eigen::Matrix<SCALAR_TYPE, 3, 1>(delta, 0, 0))
              .value() -
          this->project(homogenousPoint.block(0, 0, 3, 1) -
                        Eigen::Matrix<SCALAR_TYPE, 3, 1>(delta, 0, 0))
              .value();

      projectionJacobian->block(0, 1, 2, 1) =
          this->project(homogenousPoint.block(0, 0, 3, 1) +
                        Eigen::Matrix<SCALAR_TYPE, 3, 1>(0, delta, 0))
              .value() -
          this->project(homogenousPoint.block(0, 0, 3, 1) -
                        Eigen::Matrix<SCALAR_TYPE, 3, 1>(0, delta, 0))
              .value();
      ...
      *(projectionJacobian) /= (2 * delta);
      ...
      std::cout << "failed to numerically evaluate the projection jacobian"
                << std::endl;
      return {};
    }
  }
...
QDVO::Result<QDVO::Vector3> QDVO::EquidistantCameraModel::unproject(
    Eigen::Matrix<SCALAR_TYPE, 2, 1> pixel,
    Eigen::Matrix<SCALAR_TYPE, 2, 2> *unprojectionJacobian) const {
  Eigen::Matrix<SCALAR_TYPE, 2, 1> distortedBearing(
      (pixel(0) - this->cx) / this->fx, (pixel(1) - this->cy) / this->fy);
  ...
  for (int i = 0; i < 20; ++i)  // should converge way before 20 iters depending
                                // on table resolution
  {
