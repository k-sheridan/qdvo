...
class EquidistantCameraModel : public CameraModel {
 public:
  ...
  SCALAR_TYPE distortionFnDerivative(SCALAR_TYPE theta) const {
    // do this inline for both performance and ease.
    const SCALAR_TYPE theta2 = theta * theta;
 ...
 private:
  Eigen::Vector4d
      distortionCoeffs;  // the 3rd, 5th, 7th, and 9th order coefficients of a
                         // polynomial function of the landmark angle. Same as
                         // used in Kalibr.

  struct UniformRadiusLookUpTable {
    std::vector<SCALAR_TYPE> umap;  // r = f(\theta) = umap(\theta / resolution)
    double resolution =
        EQUIDISTANT_CAMERA_MODEL_RADIUS_MAP_RESOLUTION;  // theta =
                                                         // index*resolution
  } radiusLookUpTable;
};
}  // namespace QDVO
