...
public:
    ...
    {
        float fIndex = normalizedValue*this->n;

        int lIdx = std::floor(fIndex);
        int hIdx = std::ceil(fIndex);
        float dIdx = fIndex - lIdx;

        return 255*cv::Scalar(this->lerp(this->rLUT.at<float>(lIdx), this->rLUT.at<float>(hIdx), dIdx), this->lerp(this->gLUT.at<float>(lIdx), this->gLUT.at<float>(hIdx), dIdx), this->lerp(this->bLUT.at<float>(lIdx), this->bLUT.at<float>(hIdx), dIdx));
...
public:
    ...
    /*
     * Sets up the algorithm by adding a camera model and preallocating/precomputing.
     */
    void initialize();
