...
/**
 * The goal of this class is to be a "easily" modifiable implementation of the
 * quasi-direct approach to visual odometry. This should allow easy modification
 * of the patch comparison, and serve as a place for preallocation of
 * quantities.
 *
 * The basic datastructures are held in the graph datastructure. They contain
 * all necessary information, access, insert, and reset functions implemented in
 * a protected way.
 *
 */
class BasicPipeline {
 public:
  ...
  enum Status { INITIALIZING, TRACKING, LOST_TRACKING } status;
   ...
   */
  void addFrame(cv::Mat& image, int64_t time,
   ...
   */
  CameraModelMap::key_type addCamera(
  ...
  /**
   * Activates new landmarks from the set of keyframes such that the current
   * features are well distributed.
   */
  void activateNewLandmarks();

  /**
   * Check the state of the pipeline and determine what its status is.
   * This is ran last.
   */
   ...
   */
  void initializeCorrespondenceDistributionsForFrame(
      QDVO::Frame& frame, int level = 0,
      int searchRadius = MAXIMUM_CORRESPONDENCE_SEARCH_RADIUS);
  ...
  /**
   * Computes the pixel flow and translational pixel flow between the current
   * frame and a desired keyframe. If a pixel flow is nan, there were no
   * commonly observed features between the two frames.
   * @param keyframeKey the keyframe used to compute pixel flow with respect to.
   * @return A pair representing the pixel flow and translational pixel flow in
   * that order.
   */
  std::pair<double, double> computePixelFlowForCurrentFrame(
      KeyframeMap::key_type keyframeKey);

  /**
   * This function is ran after the current frame is setup and before the
   * correspondence distributions are initialized. It will insert a new patch
   * comparer into the unordered_map of patch comparers while removing any
   * old/redundant patch comparers.
   */
  ...
  /// global feature detector. NOT MEANT TO BE USED IN MULTIPLE THREADS WITHOUT
  /// LOCKING!
  std::unique_ptr<QDVO::FeatureDetector> featureDetector;
