...
void QDVO::PatchWarper::warpPatchToTargetFrame(QDVO::Result<Patch> &warpedPatch, Landmark &landmark, Frame &sourceFrame, Frame &targetFrame, QDVO::Graph &graph, const int patchRadius)
{
    ...
    QDVO::SE3& T_i_sourceCam = *graph.getExtrinsicMap().at(sourceFrame.extrinsicKey);
    QDVO::SE3& T_i_targetCam = *graph.getExtrinsicMap().at(targetFrame.extrinsicKey);
    ...
    auto px0 = graph.getCameraModelMap().at(targetFrame.cameraModelKey)->first->project(u0, &projJac);
    if (!px0.has_value())
    {
        return;
    }
    auto px_source = graph.getCameraModelMap().at(sourceFrame.cameraModelKey)->first->project(pt_source, &projJac);
