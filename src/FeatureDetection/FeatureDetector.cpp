...
std::vector<QDVO::Feature> QDVO::FeatureDetector::detectFeatures(
    QDVO::Frame &frame, const int level) {
  ...
  const int maxCandidatesPerSection =
      ((image.rows * image.cols) / N_SECTIONS) + 1;
  ...
  const SCALAR_TYPE gradientMagThreshold =
      (MINIMUM_NORMALIZED_GRADIENT_MAG * frame.maxIntensity());
...
#endif
  {
    PROFILE("selectGoodFeatures");

    // look for the pixels which have a sufficiently high gradient magnitude
    for (std::vector<int>::iterator upperRowBoundIt = rowBounds.begin() + 1;
         upperRowBoundIt != rowBounds.end(); ++upperRowBoundIt) {
      for (std::vector<int>::iterator upperColBoundIt = colBounds.begin() + 1;
           upperColBoundIt != colBounds.end(); ++upperColBoundIt) {
        ...
        for (int row = rl; row < ru; ++row) {
          for (int col = cl; col < cu; ++col) {
            ...
            fc.gradientNorm = sqrt(this->dxdx.at<float>(row, col) +
                                   this->dydy.at<float>(row, col));
            ...
            if (fc.harris < 0) {
              fc.score = -EDGE_WEIGHT * fc.harris;
            } else {
              fc.score = fc.harris;
            }
        ...
        for (auto e : localCandidates) {
          sum += e.gradientNorm;
          if (e.gradientNorm > maxGradientNorm) {
            maxGradientNorm = e.gradientNorm;
          }
        }
        ...
        for (auto &e : localCandidates) {
          ...
          if ((e.gradientNorm - meanGradientNorm) /
                  (maxGradientNorm - meanGradientNorm) <
              INVARIANT_THRESHOLD) {
            e.score = 0;
          }
        }
        ...
        if (localCandidates.size() >= nFeaturesPerSection) {
          std::partial_sort(
              ...
              localCandidates.begin() + nFeaturesPerSection,
              localCandidates.end(),
        ...
        for (int i = 1; i <= nFeaturesPerSection && i < localCandidates.size();
             ++i) {
          if (bestFeatureIt->score == 0) {
            continue;
          }
        ...
        for (int i = 0; i < nFeaturesPerSection && i < localCandidates.size();
             ++i) {
          std::vector<FeatureCandidate>::iterator bestFeatureIt =
              localCandidates.begin();
          bool validFeatureFound = false;
          // std::cout << "asfjdhaslkjdhf" << std::endl;
          if (this->spatialMask.at<uchar>(
                  cv::Point(bestFeatureIt->x, bestFeatureIt->y)) == 0) {
            validFeatureFound = true;
          }

          // std::cout << "kjf" << std::endl;
          for (std::vector<FeatureCandidate>::iterator it =
                   localCandidates.begin();
               it != localCandidates.end(); it++) {
          ...
          if (validFeatureFound) {
            // apply mask
            cv::circle(this->spatialMask,
                       cv::Point(bestFeatureIt->x, bestFeatureIt->y),
                       SPATIAL_MASK_RADIUS, cv::Scalar(255), -1);
