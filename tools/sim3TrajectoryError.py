...
def getInterpolatedGroundTruthPose(targetTimeNanoSeconds, groundtruth):
    ...
    for row in groundtruthReader:
        ...
        lowerBoundSO3 = quaternionToSO3(float(row[4]), float(row[5]),
    ...
    for row in groundtruthReader:
        ...
        upperBoundSO3 = quaternionToSO3(float(row[4]), float(row[5]),
                                        float(row[6]), float(row[7]))

        dt = gtTime - lowerBoundTime
        ratio = (targetTimeNanoSeconds - lowerBoundTime) / dt
        pos = lowerBoundPos + (upperBoundPos - lowerBoundPos) * ratio
        rot = lowerBoundSO3 * (sophus.SO3.exp(
            (lowerBoundSO3.inverse() * upperBoundSO3).log() * ratio))
...
def computeTrajectoryError(trajectory1, trajectory2):
    ...
    def computeError(R1, t1, R2, t2, R_sim, t_sim, scale):
        ...
        dt = (t1 - (R_sim * t2).reshape((3, 1)) * scale + t_sim).reshape(
            (3, 1))
        return dt, dR
    ...
    def computeJacobian(R1, t1, R2, t2, R_sim, t_sim, scale):
        ...
        for i in range(0, 7):
            dx = np.zeros([7, 1])
            dx[i:i + 1, 0:1] = delta
            R_simHigh, t_simHigh, scaleHigh = applyUpdate(
                R_sim, t_sim, scale, dx)
            dt, dR = computeError(R1, t1, R2, t2, R_simHigh, t_simHigh,
                                  scaleHigh)
    ...
    def computeUpdate():
        ...
        for frameNumber in trajectory1:
            ...
            dt, dR = computeError(T1['so3'], T1['pos'], T2['so3'], T2['pos'],
                                  R_sim, t_sim, scale)
            e = np.zeros([6, 1])
            e[0:3, 0:1] = dt
            e[3:6, 0:1] = dR
            J = computeJacobian(T1['so3'], T1['pos'], T2['so3'], T2['pos'],
                                R_sim, t_sim, scale)
    ...
    def computeOdometryError():
        ...
        for frameNumber in trajectory1:
            try:
                ...
                orientationErrorPerFrame['frames'][
                    frameNumber] = 180 / math.pi * np.linalg.norm(
                        (dR1.inverse() * dR2).log())
                translationErrorPerFrame['frames'][
                    frameNumber] = np.linalg.norm(dt2 - dt1)
                v1 = (dt1 / np.linalg.norm(dt1))
                v2 = (dt2 / np.linalg.norm(dt2))
                translationBearingErrorPerFrame['frames'][frameNumber] = abs(
                    180 / math.pi * math.acos(
                        np.dot(np.squeeze(np.asarray(v2)),
                               np.squeeze(np.asarray(v1)))))
                scaleErrorPerFrame['frames'][frameNumber] = np.linalg.norm(
                    dt1) / np.linalg.norm(dt2)
            except:
                continue
...
def findNewestKeyframeKey(keyframes, currentFrameKey):
    newestKey = currentFrameKey
    for key, value in keyframes.items():
        if key == currentFrameKey:
            continue
        if float(value['imustate']['time_ns']) > float(
                keyframes[newestKey]['imustate']
            ['time_ns']) and value['status'] == True:
            newestKey = key
    return newestKey


def extractImuKeyframeTrajectories(trackingData, groundtruth):
    ...
    for frameNumber in trackingData['frames']:
        ...
        currentFrameKey = str(graph['current_frame_key']['index']) + "-" + str(
            graph['current_frame_key']['generation'])
        newestKeyframe = findNewestKeyframeKey(graph['keyframes'],
                                               currentFrameKey)
        if newestKeyframe == previousKeyframe or previousKeyframe == currentFrameKey:
            continue

        imustate = graph['keyframes'][newestKeyframe]['imustate']
        imuPoseEstimates[frameNumber] = {}
        imuPoseEstimates[frameNumber]['pos'] = np.array(
            imustate['pos']['data'])
        rot = imustate['attitude']
        imuPoseEstimates[frameNumber]['so3'] = quaternionToSO3(
        ...
        try:
            ...
            print(
                f"Failed to get interpolated ground truth pose for frame {frameNumber}"
            )
