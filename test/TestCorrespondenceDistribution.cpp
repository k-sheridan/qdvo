...
class CorrespondenceDistribution : public ::testing::Test {
protected:

   std::unique_ptr<QDVO::EquidistantCameraModel>    cm = std::make_unique<QDVO::EquidistantCameraModel>(300, 301, 255, 256, PI / 2.1, 512, 400,Eigen::Vector4d(0.0034823894022493434, 0.0007150348452162257,-0.0020532361418706202, 0.00020293673591811182));

   std::shared_ptr<QDVO::PatchComparer> patchComparer =      std::make_shared<QDVO::PatchComparer>(QDVO::PatchComparer());

  std::shared_ptr<QDVO::PatchWarper> patchWarper;
};

TEST_F(CorrespondenceDistribution, Basic)
{
    ...
    auto patchResult = image.getSubPixelPatch(QDVO::Vector2(256,256), PATCH_WIDTH);
    ...
    dist.initializeDistribution(*cm, f, QDVO::LandmarkMap::key_type(), Eigen::Vector2i(256, 256), 25, patchComparer, patchResult.value());
...
TEST_F(CorrespondenceDistribution, EdgeFeature)
{
    std::shared_ptr<QDVO::RadialSearchPattern> rsp(new QDVO::RadialSearchPattern(MAXIMUM_CORRESPONDENCE_SEARCH_RADIUS));
    std::shared_ptr<QDVO::PatchComparer> patchComp(new QDVO::PatchComparer());
    QDVO::CorrespondenceDistribution dist(512, 512, rsp);
